# Content provenance

Public profile content verified on 12 September 2026 and then reconciled with the user-supplied `Olanrewaju_Eniade_UK_ATS_CV(1) (2).docx`. The supplied CV takes precedence where older public employment records differ. Raw fetched records and conversion notes remain in the ignored `.research/` directory and are excluded from the site build.

## CV reconciliation

| Item | Final portfolio content from supplied CV |
| --- | --- |
| IFAIN | Senior Data Scientist, April 2022–present |
| IHVN CaPS | Programme Manager, May 2021–March 2022 |
| Adeleke University | Adjunct Lecturer, 2020–2022; no ongoing teaching claim |
| Dementia Research Unit | Co-Investigator and Biostatistician, part-time, January 2022–April 2023 |
| University of Ibadan | Project Officer and Research Mentee, November 2019–April 2021; Assistant Statistician, part-time, July 2018–October 2019 |
| PIVOT | Statistician, May 2016–May 2018 |
| Qualifications | MSc Biostatistics 2021 and BSc Statistics 2016, University of Ibadan; ALISON Diploma in Statistics 2012 |

Per the user's explicit instruction on 12 September 2026, the public Academic Foundation card omits all years and adds an ongoing PhD in Computing at the University of Portsmouth. This current instruction supersedes the older DS-I Africa biography, which named a different university.

The September 2023–September 2024 research-affiliate entry is an additional ORCID-supported appointment, not a conflicting CV entry. The CV lists eight-plus years in its summary; the site's ten-plus years is supported by the dated May 2016 career start and the LinkedIn introduction as of September 2026. The CV supplies additional project duties, awards, memberships, tools and relocation availability.

The full CV is used as a private content source and is not included in the public repository or website. The source document remains unchanged on the user's computer.

## Identity, qualifications and career

- User-supplied LinkedIn: https://linkedin.com/in/oeniade. Public indexed introduction describes a biostatistician and data scientist with 10+ years of experience.
- User-supplied ORCID: https://orcid.org/0000-0001-7142-3818. Public API endpoints `/v3.0/0000-0001-7142-3818/person`, `/employments`, `/educations`, and `/works` were retrieved. Employment records supported the initial build and the additional research-affiliate role. The supplied CV subsequently corrected roles/dates listed above. The education endpoint is empty; qualifications are now supported directly by the CV.
- IFAIN institutional profile: https://www.ifain.org/team-member/olanrewaju-eniade/. Supports Senior Data Manager, BSc Statistics, MSc Biostatistics, University of Ibadan, research interests, and previous institutional work.
- The ORCID biography and supplied CV use “Senior Data Scientist”; the older employment record and IFAIN profile use “Senior Data Manager”. Per the user's explicit instruction on 12 September 2026, the public profile displays “Data Scientist” without “Senior”.
- Adjunct teaching overlaps other roles and ends in 2022 according to the CV. Research affiliation at IHVN is explicitly September 2023–September 2024, not ongoing.
- DS-I Africa 2023 conference biography: https://dsi-africa.org/dsi-3rd-consortium-meeting/biography/1104. Supports CAMRA data involvement, biostatistics teaching and disease modelling interests. An old doctoral-status statement has deliberately not been carried into the portfolio because no current CV or confirmation was supplied.
- Tools: R, Stata, SQL, Python, SPSS, Power BI, REDCap and ODK are in the supplied CV. Service descriptions are proposed offerings consistent with the CV and public work; they are not claims about specific paid clients or delivered outcomes. Listed global institutions are programme funders/partners, not personal consulting clients or endorsements.
- Publication-evidenced methods: the methods summary was added after reviewing the methods reported across 20 research papers/preprints in the retrieved ORCID record and the 2026 HIV funding paper. The review is documented in `METHODS-AUDIT.md`. The public wording treats these as methods demonstrated across collaborative research and does not claim sole responsibility for every analysis.
- Profile portrait: the user supplied `My picture Linkedin.jpeg` on 12 September 2026, then asked to return to the previous portrait because it appeared better. The earlier portrait and its matching social-sharing image were restored from commit `d46ede0`; the later LinkedIn image is not deployed.
- Contact number: the user explicitly supplied `+447344387533` on 12 September 2026 for public phone and WhatsApp contact. It is displayed in spaced UK format for readability while the `tel:` and `wa.me` links use the canonical digits-only form.
- Public contact email: `eniadetreasure@gmail.com`, listed as the primary verified public email in ORCID and supplied in the CV. The webpage includes the city-level location and the phone number later supplied for publication, while continuing to omit the home address and referee details.
- GitHub account: `enicrowndav`, identified through the locally authenticated Git Credential Manager account and verified against the public repositories.

## Portrait and design assets

- Actual institutional portrait: https://www.ifain.org/wp-content/uploads/2026/02/olanrewaju-1024x1024.jpeg, from the IFAIN profile. Stored locally in `assets/olanrewaju-eniade.jpg` for reliable loading. No synthetic portrait is used.
- Native SVG diagrams illustrate research workflows and evidence synthesis. They are conceptual artwork and do not represent model performance, patient data, measured effect sizes or actual study findings.
- DM Sans and Instrument Serif are self-hosted Google Fonts; their SIL Open Font License texts are in `assets/fonts/`.
- Sharing card and favicon are created specifically for this portfolio.

## Selected projects

- Predict SBI + AMR: https://github.com/enicrowndav/Predicting-SBI, public README inspected. Website description limits the claim to a research workflow; it does not imply clinical deployment, regulatory approval or verified model performance.
- CAMRA: official DS-I Africa biography linked above; no private study data is included.
- Dehydration evidence and prototype: published review https://doi.org/10.3390/healthcare13161974, IFAIN publication listing https://www.ifain.org/publications/, ORCID preprint record https://doi.org/10.20944/preprints202604.0932.v1, and public repository existence at https://github.com/enicrowndav/Risk-of-Hydration-Prediction-App. The scoring system's need for prospective validation is retained. The project card links the published review, not the preprint.

## Selected publications

All selected items have an explicit source establishing Eniade's authorship. Other staff members' publications in IFAIN's combined listing are not attributed to this profile. The site avoids total publication counts because the ORCID collection also includes preprints, an abstract and a research-tool record.

1. HIV funding modelling (2026), DOI `10.37547/tajmspr/Volume08Issue07-01`. Publisher citation record: https://www.theamericanjournals.com/index.php/tajmspr/citationstylelanguage/get/associacao-brasileira-de-normas-tecnicas?publicationId=8253&submissionId=8248. Also confirmed by the author's LinkedIn publication post and public full-text author record. Not yet present in the retrieved ORCID works collection.
2. Dehydration systematic review (2025), DOI `10.3390/healthcare13161974`. ORCID, IFAIN and PubMed PMID 40868591.
3. Hydration psychometrics (2024), DOI `10.3390/healthcare12111152`. ORCID, IFAIN and PubMed PMID 38891227.
4. HPV genital warts (2024), DOI `10.3390/v16060902`. ORCID and IFAIN.
5. COVID-19 transmission risk (2022), DOI `10.1016/j.sciaf.2022.e01334`. ORCID and IFAIN.
6. Propensity score methodology (2021), DOI `10.5539/ijsp.v10n3p154`. ORCID, IFAIN and publisher PDF.

## Remaining optional personalisation

- An explicitly approved public CV download, if desired. The current portfolio uses the CV as a private source only.
- Preferred personal domain. Do not reuse an existing organisation's domain without the owner's instruction.
- Any preferred updated portrait, service descriptions or public project highlights.
